...
// Replace the old stream endpoint and add the wizard endpoint
...
const streamRegex = /app\.post\("\/api\/campaign-builder\/stream"[\s\S]*?\}\);/m;
if (content.match(streamRegex)) {
  content = content.replace(streamRegex, newEndpoints);
  fs.writeFileSync(serverPath, content);
  console.log("Updated server.ts successfully");
} else {
  console.log("Could not find stream block");
}
